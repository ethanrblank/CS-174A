...
Declare_Any_Class( "Bee_Scene",  // An example of drawing a hierarchical object using a "model_transform" matrix and post-multiplication.
  { 'construct'( context )
      { 
		...
		camera_start = mult(camera_start, translation(0,-40,-100));
		...
		var shapes = { 'box' : new        Cube(), 
					   ...
					   'triangle': new Triangle(),
					   'square' : new Square(),
					   'tube'        : new Cylindrical_Tube  ( 30, 30 ),
        ...
        this.define_data_members( { yellow_clay: context.shaders_in_use["Phong_Model"].material( Color(  1,  1, .3, 1 ), .2, 1, .7, 40 ),
									 ...
									 greyPlastic  : context.shaders_in_use["Phong_Model" ].material( Color( .5,.5,.5, 1 ), .4, .8, .4, 20 ),  // Smaller exponent means 
									 blueGlass    : context.shaders_in_use["Phong_Model" ].material( Color( .5,.5, 1,1 ), .2, 1, 1, 40 ),
									 red    	  : context.shaders_in_use["Phong_Model" ].material( Color( .7,.0,.0, 7 ), .5, 1, .3, 20),
									 green		  : context.shaders_in_use["Phong_Model" ].material( Color(  0, 1, 0, 1),  .5, 1, .3, 20),
									 fire         : context.shaders_in_use["Funny_Shader"].material() } ); 
									 
									 
      },
	
	'draw_bee' (x, y, z, size, graphics_state)
	{
		...
		model_transform = mult(model_transform, translation(0, 20*Math.cos(1.5+t), 0));
		
		model_transform = mult(model_transform, translation(100*Math.sin(1.11*t), 0, -100*Math.cos(1.11*t)));
		
		model_transform = mult(model_transform, translation(31.5,0,0));
		model_transform = mult(model_transform, rotation(-t*31.11*2, [0,1,0]));
		model_transform = mult(model_transform, translation(-31.5,0,0));
		var mtID = model_transform;
		
		
		// STINGER
		model_transform = mult(model_transform, scale(12.5, 5.5, 5));
			this.shapes.ball.draw(graphics_state, model_transform, this.yellow_clay);
		model_transform = mult(model_transform, scale(1/12.5, 1/5.5, 1/5));
		...
		model_transform = mult(model_transform, scale(1/8, 1/4, 1/4));
		model_transform = mult(model_transform, translation(-20,0,0));
		
		// HEAD
		model_transform = mult(model_transform, translation(31.5,0,0));
		model_transform = mult(model_transform, scale(3.75, 3.75, 3.75));
			this.shapes.ball.draw(graphics_state, model_transform, this.blueGlass);
		model_transform = mult(model_transform, scale(1/3.75, 1/3.75, 1/3.75));
		model_transform = mult(model_transform, translation(-31.5,0,0));
		
		// WINGS
		model_transform = mult(model_transform, translation(20, 4.2, 12.8));
		model_transform = mult(model_transform, translation(0,0,-9));
		model_transform = mult(model_transform, rotation(-40*Math.cos(5+t), [1,0,0]));
		model_transform = mult(model_transform, translation(0,0,9));
		
	
		model_transform = mult(model_transform, scale(4, .25, 9));
			this.shapes.box.draw(graphics_state, model_transform, this.greyPlastic);
		model_transform = mult(model_transform, scale(1/4, 1/.25, 1/9));
		model_transform = mult(model_transform, translation(-20, -4.2, -12.8));
		
				// RESET
				model_transform = mtID;
		...
		model_transform = mult(model_transform, translation(16, -7, 5));

		model_transform = mult(model_transform, translation(0, 3, -1));
		model_transform = mult(model_transform, rotation(60*Math.sin(t), [1,0,0]));
		model_transform = mult(model_transform, translation(0, -3, 1));
		
		model_transform = mult(model_transform, scale(1,3,1));
		this.shapes.box.draw(graphics_state, model_transform, this.greyPlastic);
		model_transform = mult(model_transform, scale(1/1, 1/3, 1/1));

		model_transform = mult(model_transform, translation(0,-6,0));

		model_transform = mult(model_transform, translation(0, 3, -1));
		model_transform = mult(model_transform, rotation(50*Math.cos(t)*Math.cos(t), [1,0,0]));
		model_transform = mult(model_transform, translation(0, -3, 1));
		...
		model_transform = mult(model_transform, translation(20, -7, 5));
		...
		model_transform = mult(model_transform, translation(24, -7, 5));
	...
	'draw_flower' (x, y, z, size, graphics_state)
	{
		var model_transform = identity();
		var t = graphics_state.animation_time/1000;
		{
										// starting position and uniform scale
										model_transform = mult(model_transform, translation(x, y, z));
										model_transform = mult(model_transform, scale(size,size,size));
										// ... here we go
										...
										for(var i = 1; i<9; ++i) {
											if (i != 1) {
												model_transform = mult(model_transform, translation(0, 1+(i/10), 0));
												model_transform = mult(model_transform, rotation(5*Math.sin(t), [1,0,0]));
												model_transform = mult(model_transform, translation(0, -1*(1+(i/10)),0));
											}
    ...
    'display'( graphics_state )
      { 
		...
		model_transform = this.draw_flower(0,0,0,1, graphics_state);
		model_transform = this.draw_bee(0,20,0,.25, graphics_state);
		
		// GROUND
		model_transform = mult(model_transform, translation(0,3,0));
		model_transform = mult(model_transform, scale(200,0,-200));
